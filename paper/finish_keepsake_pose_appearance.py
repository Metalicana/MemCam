...
def evaluate(args, run, items, manifest, env):
    ...
    bench_path = work / "vbench.json"
    record = cached_step(bench_path, inputs)
    if record is None:
        attempt = Path(tempfile.mkdtemp(prefix="vbench_", dir=work))
        cmd = metric_command("vbench", "python", args.vbench_root / "evaluate.py",
                             "--videos_path", source, "--output_path", attempt,
                             "--mode", "custom_input", "--dimension", *DIMENSIONS)
        run_command(cmd, work / "vbench.log", env, args.vbench_root)
        values = validate_bench(attempt, [p.name for p in videos], run)
        record = {"status": "complete", "inputs": inputs,
                  "scores": {r["metric"]: r["value"] for r in values},
                  "artifacts": fingerprint([r["source"] for r in values])}
        save(bench_path, record)
...
def main():
    ...
    state = {"status": "running", "job": os.environ.get("SLURM_JOB_ID"), "duration": args.duration,
             "new_videos": 30, "reused_control_videos": 15}
