...
class LookupWorkTests(unittest.TestCase):
    ...
    def test_cli_unified_artifacts_and_attribution(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(r"MemCam & 0--45 & 11.0 & $\leq 32$ & TBD & TBD", tex)
            self.assertIn(r"MemCam & 135--180 & 46.0 & $\leq 32$ & TBD & TBD", tex)
            self.assertIn(r"WorldMem & 0--15 & 674.5 & $\leq 32$ & TBD & TBD", tex)
            self.assertIn(r"WorldMem & 45--60 & 1,124.5 & $\leq 32$ & TBD & TBD", tex)
            self.assertEqual(tex.count("& TBD & TBD"), 4)
            self.assertIn("not a measured mean", tex)
            self.assertNotIn("Early", tex)
            self.assertNotIn("Search growth", tex)
            self.assertIn("Mean query time (ms)", tex)
            self.assertIn("unmeasured cells remain TBD", tex)
            ...
            self.assertEqual(provenance["ours_table_column"]["candidate_upper_bound"], 32)
            self.assertEqual(provenance["query_latency"]["status"], "pending")
